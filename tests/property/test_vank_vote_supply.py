...
def test_recent_votes_weigh_exponentially_more():
    ...
    result = recency_tally(votes, now=10, decay=decay)
...
def test_one_vote_per_subject_enforced_in_tally():
    ...
    with pytest.raises(ValueError, match="more than once"):
        recency_tally(votes, now=2)


@pytest.mark.property
def test_future_vote_rejected():
    with pytest.raises(ValueError, match="future"):
        recency_tally([Vote("yes", "s1", 5)], now=3)
...
def test_tie_breaks_deterministically_and_margin():
    ...
    result = recency_tally(votes, now=0, decay=Decay(scale=100))
...
def test_end_to_end_register_issue_vote():
    ...
    result = recency_tally(votes, now=24, decay=Decay(num=1, den=2))
