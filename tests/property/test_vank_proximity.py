...
Principles under test:
  * A proximity proof marks a pledge "local" only when it matches the backer + the campaign's
    beacon, is in BLE range, and is co-timed with the pledge.
  * A campaign can require `min_local_backers` physically-present backers (needs a beacon).
  * Capital + breadth met but local presence missing ⇒ not funded (refund).
  * Wrong-backer / wrong-beacon proofs are rejected; out-of-range/stale ones are merely non-local.
"""
...
def test_proximity_proof_validation_and_range():
    p = attest_proximity("beacon-sq", "subjectA", beat=5, rssi_dbm=-50)
    assert p.cid.startswith("b")
    assert p.is_within_range(-90) and p.is_within_range(-50)
    assert not p.is_within_range(-40)            # -50 is weaker/farther than -40
    with pytest.raises(ValueError):
        ProximityProof(backer="a", beacon="b", beat=1, rssi_dbm=10)   # positive dBm out of range
...
def test_local_pledge_counts_when_present():
    ...
    proof = attest_proximity("beacon-sq", p.subject, beat=4, rssi_dbm=-60)
    c.pledge(p, 10, beat=5, proximity=proof)     # within window (|4-5|<=2) and range
...
def test_capital_met_but_no_local_presence_expires():
    ...
    c = Campaign(vault, beneficiary="pls1b", goal=20, deadline=10,
                 beacon="beacon-sq", min_local_backers=1, proximity_window=1)
    c.pledge(people[0], 10, beat=1)              # no proximity → remote
    c.pledge(people[1], 10, beat=2)              # no proximity → remote
...
def test_out_of_range_or_stale_proof_is_non_local_not_an_error():
    p = _person(1)
    vault = _vault_with([p])
    c = Campaign(vault, beneficiary="pls1b", goal=5, deadline=10,
                 beacon="beacon-sq", min_local_backers=1, proximity_window=1, min_rssi_dbm=-70)
    far = attest_proximity("beacon-sq", p.subject, beat=5, rssi_dbm=-95)   # weaker than -70 -> out of range
    c.pledge(p, 5, beat=5, proximity=far)        # accepted as an ordinary pledge
    assert c.backers() == 1 and c.local_backers() == 0
    assert not c.is_goal_met()                    # local presence requirement unmet
...
def test_wrong_backer_or_beacon_proof_rejected():
    p = _person(1)
    vault = _vault_with([p])
    c = Campaign(vault, beneficiary="pls1b", goal=5, deadline=10, beacon="beacon-sq")
    with pytest.raises(ValueError, match="different backer"):
        c.pledge(p, 5, beat=5, proximity=attest_proximity("beacon-sq", "someone-else", beat=5, rssi_dbm=-50))
    with pytest.raises(ValueError, match="different beacon"):
        c.pledge(p, 5, beat=5, proximity=attest_proximity("other-beacon", p.subject, beat=5, rssi_dbm=-50))
...
def test_proximity_on_beaconless_campaign_rejected():
    p = _person(1)
    vault = _vault_with([p])
    c = Campaign(vault, beneficiary="pls1b", goal=5, deadline=10)   # no beacon
    with pytest.raises(ValueError, match="no beacon"):
        c.pledge(p, 5, beat=5, proximity=attest_proximity("beacon-sq", p.subject, beat=5, rssi_dbm=-50))
...
def test_min_local_backers_requires_beacon():
    ...
    with pytest.raises(ValueError, match="requires a beacon"):
        Campaign(vault, beneficiary="pls1b", goal=5, deadline=10, min_local_backers=2)


@pytest.mark.property
def test_freeport_device_can_be_a_local_backer():
    # The freeport (IMEI) on-ramp is exactly the device that carries the BLE encounter.
    fp = register_freeport("earth", imei="IMEI-7", email="x@fp", ad_hoc_proof="vow", timestamp=1)
    vault = _vault_with([fp])
    c = Campaign(vault, beneficiary="pls1b", goal=3, deadline=10,
                 beacon="market-beacon", min_local_backers=1, proximity_window=0, min_rssi_dbm=-90)
    c.pledge(fp, 3, beat=7, proximity=attest_proximity("market-beacon", fp.subject, beat=7, rssi_dbm=-55))
    assert c.local_backers() == 1 and c.resolve(now=10).funded


@pytest.mark.property
def test_non_local_campaign_unaffected():
    # No beacon, no min_local_backers ⇒ behaves exactly like a plain campaign.
    people = [_person(1), _person(2)]
    vault = _vault_with(people)
