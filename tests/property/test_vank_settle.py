...
Principles under test:
  * The order is integer-only — a float amount is rejected before it can touch a signed record.
  * Settling moves the exact integer amount from issuer to holder via a dual-signed Knit.
  * COUPON/REDEMPTION settle native PLS; CONVERSION settles underlying-token units.
  * The value-path's own guards still apply (insufficient balance is refused; nonce advances).
  * The order has a stable content id (float-free canonical record).
"""
...
# --- the order is integer-only at the seam -----------------------------------------------
...
# --- settling moves the exact integer amount ---------------------------------------------
...
def test_conversion_settles_underlying_units_not_pls():
    # The issuer holds the underlying token; conversion delivers units, leaving PLS untouched.
    ...
    assert issuer.balance("PLS") == 100  # PLS untouched by a conversion


# --- the value-path guards still hold -----------------------------------------------------
...
# --- auditable, content-addressable order -------------------------------------------------
...
def test_order_has_stable_cid():
    ...
    assert a.cid == b.cid          # same content → same id
    assert a.cid != c.cid          # different amount → different id
